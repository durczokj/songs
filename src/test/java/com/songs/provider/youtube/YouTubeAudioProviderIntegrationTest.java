...
class YouTubeAudioProviderIntegrationTest {
    ...
    @Test
    void downloadsARealTrackWithYtDlp(@org.junit.jupiter.api.io.TempDir Path tempDir) throws Exception {
        ...
        TrackResolution resolution = new TrackResolution.Resolved(
        ...
        DownloadResult.Downloaded downloaded = assertInstanceOf(DownloadResult.Downloaded.class, result);
        assertTrue(java.nio.file.Files.isRegularFile(downloaded.outputPath()), "downloaded file should exist on disk");
        assertTrue(downloaded.outputPath().toString().endsWith(".mp3"), "downloaded file should be an mp3");
    }
