 ...
 *
 * <p>A writer must also be a reader because {@link PlaylistWriter} deliberately
 * has no URI capability method. This keeps the read/write ports separate while
 * still allowing repositories such as the local repository to support both.</p>
 */
public final class PlaylistRepositoryRegistry {
    ...
    public PlaylistReader readerFor(String uri) {
        PlaylistReader selectedReader = readers.stream()
            .filter(candidate -> candidate.canHandle(uri))
            .findFirst()
            .orElseThrow(() -> new IllegalArgumentException("No playlist reader handles URI: " + uri));
