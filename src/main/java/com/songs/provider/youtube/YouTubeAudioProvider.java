...
public class YouTubeAudioProvider implements AudioProvider {
    ...
    private static final String INNERTUBE_SEARCH_URL = "https://www.youtube.com/youtubei/v1/search?prettyPrint=false";
    ...
    public DownloadResult downloadOne(TrackResolution resolution, Path outputDir) {
        if (!(resolution instanceof TrackResolution.Resolved resolved)) {
            ...
            return new DownloadResult.DownloadFailed(resolution, "Cannot download an unresolved track: " + failed.error());
        }
        ...
        List<String> command = List.of(
            ...
            "-x",
            "--audio-format", "mp3",
            "-o", outputDir.resolve(config.outputTemplate()).toString(),
        ...
        try {
            ...
            if (!finished) {
                ...
                return new DownloadResult.DownloadFailed(resolution, "yt-dlp timed out after " + config.timeout());
            }
            if (process.exitValue() != 0) {
                String stderr = readAll(process.getErrorStream());
                logger.warn("yt-dlp failed for {} with exit code {}", resolved.track(), process.exitValue());
                return new DownloadResult.DownloadFailed(resolution, "yt-dlp exited " + process.exitValue() + ": " + stderr.trim());
            }
            ...
            if (filePath == null) {
                ...
                return new DownloadResult.DownloadFailed(resolution, "yt-dlp did not report an output file path");
            }
            ...
            return new DownloadResult.DownloadFailed(resolution, "Failed to start yt-dlp: " + e.getMessage());
