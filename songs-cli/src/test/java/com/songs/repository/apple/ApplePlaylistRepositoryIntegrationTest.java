...
class ApplePlaylistRepositoryIntegrationTest {
    ...
    void extractsMoreThan300TracksFromARealLargePlaylist() throws Exception {
        ...
        System.out.println("Fetched " + playlist.tracks().size() + " tracks for \"" + playlist.name() + "\"");
        assertTrue(playlist.tracks().size() > 300, "expected pagination to fetch beyond the first 300 tracks");

        Track firstTrack = playlist.tracks().get(0);
        assertTrue(firstTrack.isrc() != null && !firstTrack.isrc().isBlank(), "first track should have isrc populated");
        assertTrue(firstTrack.album() != null && !firstTrack.album().isBlank(), "first track should have album populated");
    }
}
