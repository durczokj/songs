...
public class ApplePlaylistPaginator {
  ...
  public Playlist fetchPlaylist(
    ...
    URI initialUri =
        URI.create(CATALOG_HOST + "/v1/catalog/" + storefront + "/playlists/" + playlistId);
  ...
  private String sendGet(URI uri, String bearerToken) throws IOException {
    ...
    if (response.statusCode() >= 400) {
      ...
      throw new IOException(
          "Catalog API request failed with HTTP " + response.statusCode() + " for URI: " + uri);
    }
  ...
  Track toTrack(JsonNode item) {
    ...
    Integer durationSec =
        attributes.has("durationInMillis")
