...
/**
 * Captures Apple Music's web-player API bearer token by loading a playlist page in a headless
 * browser and sniffing it off an outgoing request. The token is generated client-side by
 * JavaScript, so it cannot be scraped from the plain HTML/JS source.
 */
public class AppleMusicTokenProvider {
  ...
  public String captureToken(String playlistUri) {
    ...
    try {
      ...
      logger.warn(
          "Could not capture Apple Music authorization token for {}: {}",
          playlistUri,
          e.getMessage());
      return null;
    } finally {
  ...
  private String capture(String playlistUri) {
    ...
    try (Playwright playwright = Playwright.create(options)) {
      ...
      page.navigate(
          playlistUri, new Page.NavigateOptions().setWaitUntil(WaitUntilState.DOMCONTENTLOADED));
