...
class SyncJobRunnerTest {
    ...
    void resolvesBothUrisAndRunsTheTargetWriter() throws Exception {
        ...
        RecordingRepository source = new RecordingRepository(new Playlist("source", List.of(existing, added), "source"));
        RecordingRepository target = new RecordingRepository(new Playlist("target", List.of(existing), "target"));
    ...
    private static final class RecordingRepository implements PlaylistReader, PlaylistWriter {
        ...
        public List<RemoveResult> removeTracks(String uri, List<Track> tracks) {
            return tracks.stream().map(RemoveResult.Removed::new).map(result -> (RemoveResult) result).toList();
        }
    }
}